...
const Printer = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await axios.get("https://umashankar-be.onrender.com/printer");
    ...
    return (
      <div className="min-h-screen px-4 sm:px-8 lg:px-16 py-8 pb-16">
        <div className="mb-10">
          ...
          <div className="h-3 bg-slate-200 rounded w-56 mb-4 animate-pulse" />
  ...
  return (
    <div className="min-h-screen px-4 sm:px-8 lg:px-16 py-8 pb-16">
      {featured.length > 0 && (
        <div className="mb-10">
          <h2 className="text-lg font-bold text-slate-800 mb-1">Featured Services</h2>
          <p className="text-sm font-semibold text-slate-500 mb-4">
            Specialist services we are known for
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {featured.map((item, index) => (
              <Cards key={index} title={item.title} description={item.description} image={item.image} featured={true} />
            ))}
          </div>
        </div>
      )}
      <div>
        <h2 className="text-lg font-bold text-slate-800 mb-1">Products & Repairs</h2>
        <p className="text-sm font-semibold text-slate-500 mb-4">
